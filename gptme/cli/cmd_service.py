"""
Scaffold a minimal persistent headless gptme agent as a systemd user service
(Linux) or launchd agent (macOS).

This command generates the minimal files (unit file, startup script, config)
to run a gptme agent on a timer. It does NOT include:
- Task management, lessons, or work supply infrastructure
- Autonomous run loop, context generation, or hooks
- Example or utility scripts

For a complete, batteries-included agent workspace with all of the above, see:
...
#
# For a full-featured agent with task management, lessons, and work
# supply infrastructure, see: https://github.com/gptme/gptme-agent-template
...
# Create session log directory and run the agent
mkdir -p "$JOURNAL_DIR"
...
{{
  ...
  echo "## Work"
  echo
  echo "This scaffold has no work loop. See gptme-agent-template for full"
  echo "implementation (cascade selector, task executor, etc.)."
  echo
}} >> "$SESSION_LOG"

echo "gptme agent {name}: session log at $SESSION_LOG"
"""

GPTME_TOML_TEMPLATE = """\
# Minimal gptme configuration for {name}
# For full agent configuration with context, lessons, and hooks, see:
# https://github.com/gptme/gptme-agent-template/blob/master/gptme.toml
...
For the full agent workspace template with examples, best practices, and
hooks, see: https://github.com/gptme/gptme-agent-template

## Role

{name} is a persistent headless gptme agent. It runs on a systemd/launchd timer
and executes its work loop non-interactively.

## Core Rules

### 1. Absolute Paths

Use `git rev-parse --show-toplevel` for the repo root — never hardcode paths.

### 2. Conventional Commits

Use gptme's commit format (see gptme-agent-template/ABOUT.md):
- `feat:` — new feature
- `fix:` — bug fix
- `docs:` — documentation
...
This scaffold is minimal. To add autonomous run infrastructure, tasks,
lessons, and more, see: https://github.com/gptme/gptme-agent-template/
"""
