...
def _is_harness_user_message(m: "Message") -> bool:
    """Whether a user-role message was generated by the harness, not a human.

    Every harness-injected user message is wrapped in a ``<system>`` tag
    (auto-reply nudges, the stuck-loop nudge) or carries the verification-failure
    marker. The wrapper is reserved for harness output, so it — not the
    auto-reply marker alone — is what distinguishes harness from human: a real
    user message can *quote* the marker (e.g. asking about the nudge) without
    being wrapped.
    """
    content = m.content or ""
    return content.startswith(("<system>", _VERIFY_FAILED_MARKER))


def _is_auto_reply_message(m: "Message") -> bool:
    """Whether a user message is a harness auto-reply nudge.

    Narrower than :func:`_is_harness_user_message`: the consecutive auto-reply
    counter only counts the "No tool call detected" nudges, so the stuck-loop
    nudge must not inflate it.
    """
    content = m.content or ""
    return content.startswith("<system>") and _AUTO_REPLY_MARKER in content
...
def current_episode(messages: "list[Message]") -> "list[Message]":
    ...
    user message is new authority and starts a fresh episode — even when it
    quotes the auto-reply marker text.
    """
    for i in range(len(messages) - 1, -1, -1):
        m = messages[i]
        if m.role == "user" and not _is_harness_user_message(m):
            return messages[i + 1 :]
...
def auto_reply_hook(
    ...
    # Count consecutive auto-replies. Only actual harness nudges count: a real
    # user message that merely quotes the marker must not be mistaken for one.
    auto_reply_count = 0
    for msg in reversed(manager.log.messages):
        if msg.role == "user" and _is_auto_reply_message(msg):
            auto_reply_count += 1
        elif msg.role == "assistant":
            # Stop counting when we hit an assistant message with tools
            if list(ToolUse.iter_from_content(msg.content)):
...
def block_budget_hook(
    ...
    if chat is None and (logdir := getattr(manager, "logdir", None)) is not None:
        # Server steps run with the process-level workspace config, whose
        # ``chat`` is None; the conversation's own config (including its
        # ``interactive`` flag) lives in its logdir. Resolve it so a human-driven
        # server/web session is not force-ended as if it were headless.
        from ..config import ChatConfig  # fmt: skip

        chat = ChatConfig.from_logdir(logdir)
    # ``chat is None`` means no Chat object and no conversation config
    # (library/headless usage) — there is no human at a terminal, so treat it
    # as non-interactive.
