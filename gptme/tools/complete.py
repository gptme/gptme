...
def _is_harness_user_message(m: "Message") -> bool:
    ...
    Harness user messages are ``<system>``-wrapped auto-reply/nudge messages or
    verification-failure results. The auto-reply marker alone is not sufficient:
    a real user message can *quote* the marker (e.g. asking about the nudge),
    so the ``<system>`` wrapper is what distinguishes harness from human.
    """
    content = m.content or ""
    return (content.startswith("<system>") and _AUTO_REPLY_MARKER in content) or (
        content.startswith(_VERIFY_FAILED_MARKER)
    )
