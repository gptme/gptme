    ...
    def convert(
        ...
        **kwargs,
        ...
        if avail.pdftoppm:
            ...
            out_prefix = dest.parent / dest.stem
            # Snapshot pre-existing files so the fallback glob can't pick up stale ones
            stem_esc = _glob.escape(dest.stem)
            pre_existing = set(dest.parent.glob(f"{stem_esc}*"))
            cmd = ["pdftoppm", fmt_flag, "-r", str(dpi), str(src), str(out_prefix)]
            result = subprocess.run(cmd, capture_output=True, check=False)
            if result.returncode == 0:
                # Collect output files generated by this invocation only
                candidates = sorted(
                    p
                    for p in dest.parent.glob(f"{stem_esc}-*.{dest_ext}")
                ...
                if not candidates:
                    ...
                    candidates = sorted(
                        p
                        for p in dest.parent.glob(
                            f"{stem_esc}*" if dest_ext == "png" else f"{stem_esc}*.jpg"
                        )
                        if p not in pre_existing
                    )
                if candidates:
                    candidates[0].rename(dest)
                    # Remove extra page files to avoid undeclared artifacts
