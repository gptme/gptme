...
def _get_temperature(
    ...
    All others use caller-supplied temperature, falling back to the global default.
    """
    if provider == "moonshot":
        return 1.0
    if model_meta is not None and "gpt-5" in model_meta.model:
        return 1.0
    return caller_temp if caller_temp is not None else TEMPERATURE


def _get_top_p(
    provider: Provider,
    model_meta: "ModelMeta | None" = None,
    caller_top_p: float | None = None,
) -> float | None:
    """Return the top_p for a given provider.

    Moonshot/Kimi models require top_p=0.95. All others use caller-supplied top_p,
    ...
    if provider == "moonshot":
        return 0.95
    return caller_top_p if caller_top_p is not None else TOP_P
    ...
    if _should_use_responses_api(provider, model_meta, client):
        ...
        if not is_reasoner:
            response_kwargs["temperature"] = _get_temperature(
                provider, model_meta, temperature
            )
            top_p = _get_top_p(provider, model_meta, top_p)
...
def _stream_responses(
    ...
    if not is_reasoner:
        kwargs["temperature"] = _get_temperature(provider, model_meta, temperature)
        top_p = _get_top_p(provider, model_meta, top_p)
        if top_p is not None:
            kwargs["top_p"] = top_p
