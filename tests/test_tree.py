def _git_cmd_contains(cmd, *parts):
    """Check if a command list contains the git subcommand parts."""
    if not isinstance(cmd, list) or len(cmd) < len(parts):
        return False
    # Check if the git subcommand parts appear in the command
    # (accounting for git config flags that come before the subcommand)
    return parts == tuple(cmd[-len(parts) :])
...
def test_get_tree_output_different_methods(tmp_path, monkeypatch):
    ...
    for method_str in ["tree", "git", "ls"]:
        ...
        if method == "tree":
            ...
            assert any(
                "ls-files" in cmd for cmd in called_commands if isinstance(cmd, list)
            )
        elif method == "ls":
            assert any(
                cmd[:2] == ["ls", "-R"]
...
def test_get_tree_output_fallback_when_tree_missing(tmp_path, monkeypatch):
    ...
    def mock_run(*args, **kwargs):
        ...
        if isinstance(args[0], list) and "ls-files" in args[0]:
            return subprocess.CompletedProcess(
    ...
    assert any(isinstance(cmd, list) and "ls-files" in cmd for cmd in called_commands)
...
def test_get_tree_output_not_git_repo(tmp_path, monkeypatch):
    ...
    assert not any(
        isinstance(cmd, list) and "ls-files" in cmd for cmd in called_commands
    )
