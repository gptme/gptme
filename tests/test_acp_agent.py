...
class TestReportToolCall:
    ...
    @pytest.mark.asyncio()
    async def test_report_stores_tool_call(self, agent_with_conn):
        tc = ToolCall(
            tool_call_id="call_test123",
            title="Test call",
            kind=ToolKind.EXECUTE,
        )
        await agent_with_conn._report_tool_call("session_1", tc)

        assert "session_1" in agent_with_conn._tool_calls
        assert "call_test123" in agent_with_conn._tool_calls["session_1"]

    @pytest.mark.asyncio()
    async def test_report_sends_update(self, agent_with_conn):
        tc = ToolCall(
            tool_call_id="call_test123",
            title="Test call",
            kind=ToolKind.EXECUTE,
        ...
        agent_with_conn._conn.session_update.assert_awaited_once_with(
    ...
    @pytest.mark.asyncio()
    async def test_report_no_connection(self):
        ...
        # Should not raise
        await agent._report_tool_call("session_1", tc)
        assert "session_1" not in agent._tool_calls

    @pytest.mark.asyncio()
    async def test_report_multiple_calls_same_session(self, agent_with_conn):
        ...
        await agent_with_conn._report_tool_call("session_1", tc1)
        await agent_with_conn._report_tool_call("session_1", tc2)

        assert len(agent_with_conn._tool_calls["session_1"]) == 2
...
class TestUpdateToolCall:
    ...
    @pytest.mark.asyncio()
    async def test_update_status(self, agent_with_tool):
        await agent_with_tool._update_tool_call(
            "session_1", "call_existing", ToolCallStatus.IN_PROGRESS
        )

        tc = agent_with_tool._tool_calls["session_1"]["call_existing"]
        assert tc.status == ToolCallStatus.IN_PROGRESS

    @pytest.mark.asyncio()
    async def test_update_with_content(self, agent_with_tool):
        content = [{"type": "text", "text": "result"}]
        await agent_with_tool._update_tool_call(
            "session_1", "call_existing", ToolCallStatus.COMPLETED, content=content
        )
    ...
    @pytest.mark.asyncio()
    async def test_update_sends_session_update(self, agent_with_tool):
        await agent_with_tool._update_tool_call(
            "session_1", "call_existing", ToolCallStatus.COMPLETED
        )

        agent_with_tool._conn.session_update.assert_awaited_once()
        call_args = agent_with_tool._conn.session_update.call_args
    ...
    @pytest.mark.asyncio()
    async def test_update_no_connection(self):
        agent = GptmeAgent()
        # Should not raise
        await agent._update_tool_call("session_1", "call_x", ToolCallStatus.COMPLETED)
...
class TestCompletePendingToolCalls:
    ...
    @pytest.fixture()
    def agent_with_mixed_calls(self):
        agent = GptmeAgent()
        conn = MagicMock()
        conn.session_update = AsyncMock()
        agent._conn = conn
    ...
    @pytest.mark.asyncio()
    async def test_completes_pending_and_in_progress(self, agent_with_mixed_calls):
        await agent_with_mixed_calls._complete_pending_tool_calls("session_1")

        calls = agent_with_mixed_calls._tool_calls["session_1"]
    ...
    @pytest.mark.asyncio()
    async def test_complete_with_failure(self, agent_with_mixed_calls):
        await agent_with_mixed_calls._complete_pending_tool_calls(
            "session_1", success=False
        )
    ...
    @pytest.mark.asyncio()
    async def test_no_tool_calls_for_session(self):
        agent = GptmeAgent()
        conn = MagicMock()
        conn.session_update = AsyncMock()
        agent._conn = conn
        ...
        await agent._complete_pending_tool_calls("nonexistent_session")
...
class TestRequestToolPermission:
    ...
    @pytest.mark.asyncio()
    async def test_no_connection_auto_allows(self):
        agent = GptmeAgent()
        tc = ToolCall(tool_call_id="call_1", title="Test", kind=ToolKind.EXECUTE)
        result = await agent._request_tool_permission("session_1", tc)
        assert result is True

    @pytest.mark.asyncio()
    async def test_cached_allow_policy(self):
        agent = GptmeAgent()
    ...
    @pytest.mark.asyncio()
    async def test_cached_reject_policy(self):
        agent = GptmeAgent()
        agent._conn = MagicMock()
    ...
    @pytest.mark.asyncio()
    async def test_allow_once_response(self):
        agent = GptmeAgent()
        conn = MagicMock()
    ...
    @pytest.mark.asyncio()
    async def test_allow_always_response(self):
        agent = GptmeAgent()
        conn = MagicMock()
        conn.request_permission = AsyncMock(
    ...
    @pytest.mark.asyncio()
    async def test_reject_always_response(self):
        agent = GptmeAgent()
        conn = MagicMock()
        conn.request_permission = AsyncMock(
    ...
    @pytest.mark.asyncio()
    async def test_cancelled_response(self):
        agent = GptmeAgent()
        conn = MagicMock()
        conn.request_permission = AsyncMock(
    ...
    @pytest.mark.asyncio()
    async def test_permission_request_exception_auto_allows(self):
        agent = GptmeAgent()
        conn = MagicMock()
...
class TestInitializeWithoutAcp:
    ...
    @pytest.mark.asyncio()
    async def test_initialize_without_acp_package(self):
    ...
    @pytest.mark.asyncio()
    async def test_load_session_not_implemented(self):
        agent = GptmeAgent()
        with pytest.raises(NotImplementedError, match="not yet implemented"):
            await agent.load_session(session_id="fake_session")
